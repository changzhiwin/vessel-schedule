...
trait SubscriberTb {
  ...
  def get(id: UUID): Task[Subscriber]
...
final case class SubscriberTbLive(
  ...
  def get(id: UUID): Task[Subscriber] = 
    // TODO: pattern var idiomContext in method get is never used: use a wildcard `_` or suppress this warning with `idiomContext@_`
    run(
      query[Subscriber]
        .filter(s => s.id == lift(id))
    )
    .map(_.headOption)
    .someOrFail(DbRecordNotFound(s"Subscriber#id ${id.toString}"))
    .implicitly
